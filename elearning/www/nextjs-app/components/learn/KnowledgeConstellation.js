...
const KnowledgeConstellation = ({ 
  ...
  const handleBackToTopics = () => {
    ...
    const reloadTopicsData = async () => {
      setIsLoadingTopics(true);
      try {
        const response = await fetch('/api/method/elearning.elearning.doctype.student_topic_mastery.student_topic_mastery.get_knowledge_constellation', {
          method: 'GET',
      ...
      <div className="absolute top-4 left-4 z-40 bg-white/90 backdrop-blur-sm rounded-xl px-4 py-2 shadow-lg border border-gray-200/50">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-semibold text-gray-700">
            {currentView === 'topics' ? 'Tổng quan Chòm sao' : `Cây tri thức - ${selectedTopic?.topic_name || 'Chi tiết'}`}
          </span>
        </div>
        <div className="text-xs text-gray-500 mt-1">
          {currentView === 'topics' ? 'Nhấp vào ngôi sao để xem chi tiết' : 'Learning Objects và mối quan hệ'}
        </div>
      </div>
